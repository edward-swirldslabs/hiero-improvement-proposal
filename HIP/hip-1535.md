...
Hiero SDKs SHOULD add typed support for the CLPR Service HAPI transactions (§12.2) and queries (§12.3) —
Channel registration, completion, and closure; Connector registration, completion, and deregistration; bundle
submission; and ledger-configuration updates — consistent with how SDKs wrap other native services.

Sending a message is not a HAPI transaction. Applications are smart contracts and send messages by calling
`sendMessage` on the CLPR system contract (§12.8); responses and inbound messages are delivered to them as EVM
callbacks. SDK support for sending therefore consists of ordinary contract-call and ABI helpers rather than a
dedicated transaction type. Endpoint-to-endpoint gRPC sync traffic (§12.7) is a peer-to-peer protocol between node
software, not an SDK-facing surface.

### 12. Protobuf and API Reference

This section indexes the wire-level definitions of the Hiero reference implementation. The linked sources on the
`main` branch of [hiero-consensus-node](https://github.com/hiero-ledger/hiero-consensus-node) are normative for
field names, field numbers, and enum ordinals; the surrounding text explains how each definition relates to the
concepts in §4–§9. All protobuf paths below are relative to
[`hapi/hedera-protobuf-java-api/src/main/proto`][proto-root]. The implementation currently carries protobuf for
message redaction (`ClprRedactMessageTransactionBody`, `ClprRedactedMessage`, and related functionality and
response codes); redaction is a rejected idea (see [Rejected Ideas](#rejected-ideas)), those definitions are not
part of this specification, and they are expected to be removed.

#### 12.1 CLPR Service (HAPI gRPC)

[`services/clpr_service.proto`][clpr_service] defines `proto.ClprService`, the HAPI gRPC service through which
clients submit CLPR transactions and queries to consensus nodes. Like other HAPI services, every transaction RPC
accepts a signed `Transaction` and returns a `TransactionResponse` precheck result; every query RPC accepts a
`Query` and returns a `Response`.

#### 12.2 Transactions

| Transaction body | Source | Purpose (spec section) |
|---|---|---|
| `ClprRegisterChannelTransactionBody` | [`services/clpr_register_channel.proto`][register_channel] | Commit phase of Channel creation; claims a Channel ID in `PENDING` state (§4.4). |
| `ClprCompleteChannelTransactionBody` | [`services/clpr_complete_channel.proto`][complete_channel] | Reveal phase of Channel creation; binds the verifier and submits the peer configuration and manifest proofs for `verifyConfig` (§4.4, §5). Also defines `ClprSignatureScheme`. |
| `ClprCloseChannelTransactionBody` | [`services/clpr_close_channel.proto`][close_channel] | Initiates graceful Channel closure (§4.4, §9). |
| `ClprSubmitBundleTransactionBody` | [`services/clpr_submit_bundle.proto`][submit_bundle] | Permissionless submission of a peer bundle for verification, dispatch, and payment (§6.2–§6.4, §7.3). |
| `ClprRegisterConnectorTransactionBody` | [`services/clpr_register_connector.proto`][register_connector] | Commit phase of Connector registration on a Channel (§7.1). |
| `ClprCompleteConnectorTransactionBody` | [`services/clpr_complete_connector.proto`][complete_connector] | Reveal phase of Connector registration; funds the Connector and binds its cross-chain `connector_id` (§7.1). |
| `ClprDeregisterConnectorTransactionBody` | [`services/clpr_deregister_connector.proto`][deregister_connector] | Removes a Connector and releases its remaining balance (§7.1, §8.3). |
| `ClprUpdateLedgerConfigurationTransactionBody` | [`services/clpr_update_ledger_configuration.proto`][update_ledger_config] | CLPR Service Admin update of the local ledger configuration and throttles (§4.2, §8.6). |

#### 12.3 Queries

| Query / response | Source | Purpose |
|---|---|---|
| `ClprGetLedgerConfigurationQuery` / `Response` | [`services/clpr_get_ledger_configuration.proto`][get_ledger_config] | Returns the local ledger's current CLPR configuration (§4.2). |
| `ClprGetEndpointManifestQuery` / `Response` | [`services/clpr_get_endpoint_manifest.proto`][get_endpoint_manifest] | Returns the local ledger's current endpoint manifest (§4.3). |

#### 12.4 HAPI envelope and enum additions

CLPR extends the shared HAPI envelopes rather than defining new ones:

| Envelope | Source | CLPR additions |
|---|---|---|
| `TransactionBody.data` | [`services/transaction.proto`][transaction] | Fields `82`–`86` and `88`–`91` (the §12.2 bodies plus the node-generated `clpr_endpoint_publication` of §12.5). |
| `Query.query` | [`services/query.proto`][query] | Fields `59` (`clprGetLedgerConfiguration`) and `60` (`clprGetEndpointManifest`). |
| `Response.response` | [`services/response.proto`][response] | Fields `159` and `160`, the corresponding query responses. |
| `HederaFunctionality` | [`services/basic_types.proto`][basic_types] | `ClprUpdateLedgerConfiguration` (`117`) through `ClprGetEndpointManifest` (`128`), used for throttling, fees, and permissions. |
| `ResponseCodeEnum` | [`services/response_code.proto`][response_code] | The `CLPR_*` codes beginning at `CLPR_NOT_ENABLED` (`537`), covering precheck and handle failures for every CLPR transaction and query. |

#### 12.5 Node-generated transactions

[`services/auxiliary/clpr/clpr_endpoint_publication.proto`][endpoint_publication] defines
`ClprEndpointPublicationTransactionBody`, which a consensus node creates and submits itself to publish its own
CLPR endpoint into the in-progress manifest construction ([`services/state/clpr/clpr_endpoint_manifest_construction.proto`][manifest_construction]).
Once the construction completes, the result becomes the ledger's endpoint manifest (§4.3). Users do not submit
this transaction.

#### 12.6 Ledger configuration and verifier payloads

[`services/state/clpr/clpr_ledger_configuration.proto`][ledger_config] defines `ClprLedgerConfiguration`, the
ledger identity and configuration exchanged with peers and proven by `verifyConfig` (§4.2, §5). It also defines
`ClprThrottles` (per-Channel limits such as maximum payload size and per-message gas) and the `ClprEndpoint` /
`ClprServiceEndpoint` types used in the endpoint manifest.

The contents of configuration and bundle proofs depend on the peer's proof system. The reference implementation
defines protobuf payloads for the non-Hiero verifiers it ships:

- [`services/state/clpr/clpr_qbft_ledger_configuration_payload.proto`][qbft_payload]: Besu QBFT block headers and
  storage proofs.
- [`services/state/clpr/clpr_sei_ledger_configuration_payload.proto`][sei_payload]: Sei (CometBFT/IAVL) signed
  headers, validator sets, state proofs, trust anchors, and bundle payloads.

These payloads are defined by each verifier, not by the CLPR Service (§5). They are listed here only for
reference.

#### 12.7 Endpoint-to-endpoint wire protocol

Endpoints (§6.2) exchange data over the gRPC service `proto.ClprEndpointService`, which is separate from the HAPI
service of §12.1. It has three methods: `sync` (unary), `streamingSync` (bidirectional streaming), and
`discoverEndpoints` (unary). The reference implementation registers these methods directly rather than generating
them from a `.proto` service declaration, but every request and response is a protobuf message:

- [`services/state/clpr/clpr_message.proto`][clpr_message]: `ClprSyncPayload`, `ClprBundleRequest`,
  `ClprBundleResponse`, and `ClprStreamingSyncPayload` carry queue progress and bundles between endpoints
  (§6.2). The same file defines the queued message model shared by the wire protocol and state (`ClprMessage`,
  `ClprMessageReply`, `ClprMessageReplyStatus`, `ClprControlMessage`, `ClprConfigUpdate`, `ClprMessagePayload`,
  and `ClprQueueMetadata`; §6.1, §6.4).
- [`services/state/clpr/clpr_discovery.proto`][clpr_discovery]: `ClprDiscoverEndpointsRequest` and
  `ClprDiscoverEndpointsResponse`, which endpoints use to learn a peer's current endpoints (§4.3, §9).
- [`services/state/clpr/clpr_peer_endpoints.proto`][peer_endpoints]: the peer endpoint set that each endpoint
  keeps.
- [`services/state/clpr/clpr_bundle_content.proto`][bundle_content]: `ClprBundleContent`, the verified contents of
  a bundle after `verifyBundle` returns (§6.3).

#### 12.8 Smart contract interface

Applications, and verifiers on Hiero, interact with CLPR through EVM system contracts rather than protobuf:

| Address | System contract | Functions |
|---|---|---|
| `0x16e` | CLPR | `sendMessage(bytes32 channelId, bytes32 connectorId, bytes targetApplication, bytes messageData) returns (uint64 messageId)`; `getChannelQueueState(bytes32) returns (uint64, uint64)`; `getEndpointManifest() returns (bytes)`; plus `verifyConfig(bytes)` and `verifyBundle(bytes, bytes)` for Hiero peers. |
| `0x16f` | Besu QBFT verifier | `verifyConfig(bytes)`, `verifyBundle(bytes, bytes)` |
| `0x170` | Sei verifier | `verifyConfig(bytes)`, `verifyBundle(bytes, bytes)` |
| `0x171` | Ethereum verifier | `verifyConfig(bytes)`, `verifyBundle(bytes, bytes)` |

`sendMessage` is the only way to send a message (§7.2); it enqueues the message on the named Channel and returns
its message ID. Target applications MUST implement two callbacks, which the CLPR Service invokes as described in
§7.3:

- `onClprMessage(bytes32 channelId, bytes sender, bytes messageData)` (selector `0x2ab00809`), which delivers an
  inbound Data Message.
- `onClprResponse(bytes32 channelId, uint64 messageId, uint8 status, bytes data)` (selector `0x3b74550e`), which
  delivers the response to a message the application sent earlier. `messageId` is the ID returned by `sendMessage`,
  and `status` is a `ClprMessageReplyStatus` ordinal.

#### 12.9 CLPR Service state

The CLPR Service keeps its state in ordinary Merkle state. Each state has a `StateIdentifier` in
[`block/stream/output/state_changes.proto`][state_changes], and the key and value types used in the virtual map
are declared in [`platform/state/virtual_map_state.proto`][virtual_map_state]:

| ID | State | Type | Key | Value |
|---|---|---|---|---|
| `59` | `STATE_ID_CLPR_LEDGER_CONFIGURATION` | Singleton | — | `ClprLedgerConfiguration` |
| `60` | `STATE_ID_CLPR_CHANNELS` | Key/value | Channel ID (`ProtoBytes`) | `ClprChannel` ([`clpr_channel.proto`][clpr_channel]) |
| `61` | `STATE_ID_CLPR_PENDING_COMMITMENTS` | Key/value | Commitment (`ProtoBytes`) | `ProtoBytes` |
| `62` | `STATE_ID_CLPR_MESSAGE_QUEUE` | Key/value | `ClprMessageKey` (channel ID + message ID) | `ClprMessageValue` (payload + running hash) |
| `63` | `STATE_ID_CLPR_CONNECTORS` | Key/value | `ClprConnectorKey` (channel ID + connector ID) | `ClprConnector` ([`clpr_connector.proto`][clpr_connector]) |
| `64` | `STATE_ID_CLPR_PENDING_CONNECTOR_COMMITMENTS` | Key/value | Commitment (`ProtoBytes`) | `ProtoBytes` |
| `65` | `STATE_ID_CLPR_SUBMISSION_THROTTLES` | Key/value | Endpoint account (`AccountID`) | `ThrottleUsageSnapshot` |
| `66` | `STATE_ID_CLPR_ENDPOINT_MANIFEST` | Singleton | — | `ClprEndpointManifest` ([`clpr_endpoint_manifest.proto`][endpoint_manifest]) |
| `67` | `STATE_ID_CLPR_ENDPOINT_MANIFEST_CONSTRUCTION` | Singleton | — | `ClprEndpointManifestConstruction` |

`ClprChannel` holds the Channel's status (`ClprChannelStatus`), bound verifier, current trust anchor, peer
configuration, and queue progress (§4.4, §6.2). `ClprConnector` holds a Connector's owner, balance, and
cross-chain identity (§7.1).

#### 12.10 Block stream

CLPR adds no new block item types. CLPR activity appears in the block stream through the standard
`SignedTransaction`, `TransactionResult`, `TransactionOutput`, `TraceData`, and `StateChanges` items, using the
state identifiers and value types listed in §12.9. Indexers, including mirror nodes (§10), should understand the
following structure.

**Bundle transactions and their children.** A `ClprSubmitBundle` transaction at consensus time `T` is a user
transaction. When it handles the bundle, each Data Message that is dispatched to an application (§7.3) and each
response delivered through `onClprResponse` becomes a separate child `ContractCall` dispatch. The resulting
block items are:

```
T      SignedTransaction   ClprSubmitBundle
       TransactionResult
       StateChanges @T     every map and queue change made by the bundle AND all of its children
T+1    SignedTransaction   synthetic ContractCall to the first target application
       TransactionResult   parent_consensus_timestamp = T; this call's status
       TransactionOutput   contract call result
       TraceData           EVM trace (logs, storage reads)
T+2…   the same for each later dispatch, in the order the bundle handled them
```

Each child's calldata (`onClprMessage` or `onClprResponse`), status, gas usage, and EVM logs are visible on their
own. However, the state changes of the whole bundle, including the applications' contract storage and account
balances, are emitted together in the single `StateChanges` item at `T`. The block stream does not attribute state
changes to individual children or messages. Connector charges and reimbursement of the submitter (§7.3) appear in
the bundle transaction's record.

**Correlating children with messages.** A bundle does not dispatch every message it carries. Control Messages,
messages from missing or underfunded Connectors, and messages already handled by an earlier bundle are not
dispatched, so child records cannot be matched to message IDs by position alone. The authoritative outcome of each
received message is the `ClprMessageReply` (with its `ClprMessageReplyStatus`) that the bundle enqueues in
`STATE_ID_CLPR_MESSAGE_QUEUE`. Those entries are part of the `StateChanges` at `T` and are keyed by
`ClprMessageKey` (channel ID + message ID).

**Multiple bundles in one block.** Each bundle is its own user transaction with its own consensus timestamp and
`StateChanges` item. Channel, message queue, and Connector changes can therefore be attributed to the bundle that
made them, even when several bundles on different Channels are handled in the same block. Singleton states are the
exception (`STATE_ID_CLPR_LEDGER_CONFIGURATION`, `STATE_ID_CLPR_ENDPOINT_MANIFEST`, and
`STATE_ID_CLPR_ENDPOINT_MANIFEST_CONSTRUCTION`). As for all Hiero singletons, only their final value is emitted,
once, at the end of the block, so a change to them cannot be tied to a specific transaction through
`StateChanges`.

[proto-root]: https://github.com/hiero-ledger/hiero-consensus-node/tree/main/hapi/hedera-protobuf-java-api/src/main/proto
[clpr_service]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_service.proto
[register_channel]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_register_channel.proto
[complete_channel]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_complete_channel.proto
[close_channel]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_close_channel.proto
[submit_bundle]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_submit_bundle.proto
[register_connector]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_register_connector.proto
[complete_connector]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_complete_connector.proto
[deregister_connector]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_deregister_connector.proto
[update_ledger_config]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_update_ledger_configuration.proto
[get_ledger_config]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_get_ledger_configuration.proto
[get_endpoint_manifest]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/clpr_get_endpoint_manifest.proto
[transaction]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/transaction.proto
[query]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/query.proto
[response]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/response.proto
[basic_types]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/basic_types.proto
[response_code]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/response_code.proto
[endpoint_publication]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/auxiliary/clpr/clpr_endpoint_publication.proto
[manifest_construction]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_endpoint_manifest_construction.proto
[ledger_config]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_ledger_configuration.proto
[qbft_payload]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_qbft_ledger_configuration_payload.proto
[sei_payload]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_sei_ledger_configuration_payload.proto
[clpr_message]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_message.proto
[clpr_discovery]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_discovery.proto
[peer_endpoints]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_peer_endpoints.proto
[bundle_content]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_bundle_content.proto
[clpr_channel]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_channel.proto
[clpr_connector]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_connector.proto
[endpoint_manifest]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/services/state/clpr/clpr_endpoint_manifest.proto
[state_changes]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/block/stream/output/state_changes.proto
[virtual_map_state]: https://github.com/hiero-ledger/hiero-consensus-node/blob/main/hapi/hedera-protobuf-java-api/src/main/proto/platform/state/virtual_map_state.proto
...
The reference implementation of the Hiero CLPR Service is in
[hiero-consensus-node](https://github.com/hiero-ledger/hiero-consensus-node). It was introduced in
[PR #27029](https://github.com/hiero-ledger/hiero-consensus-node/pull/27029) and includes the CLPR Service module
(`hedera-node/hedera-clpr-service` and `hedera-node/hedera-clpr-service-impl`), the endpoint client, the CLPR and
verifier system contracts, and the protobuf definitions indexed in §12.
